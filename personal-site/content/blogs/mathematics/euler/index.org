...
Recently Po Shen Loh released a preprint critiquing how American schools
...
In contrsat, other famous irrational numbers like \(\pi\) or even
...
So, we have a definition of \(e\) as it relates to the area under a
hyperbola. What does that have to do with exponentiation?

At this point, nothing--and that's normal. The fact that \(\pi\) is defined by
circle geometry doesn't immediately connect to side length ratios of right
triangles either. To make this connection, we have to actually study the
trigonometric functions, not just define them like in \eqref{eq:pidef}. For our
case, let's study \(\exp\) and \(\ln\).

It turns out that logarithms have a surprising property: if we rescale the
length scale of the hyperbola in \eqref{eq:logint} by \(a\), the area scales as
\begin{align}
\int_{x_{1}}^{x_{2}} \frac{1}{y} \dd{y} = \int^{a x_{2}}_{a x_{1}} \frac{1}{y}\dd{y}
\end{align}
...
curriculum, they can be introduced to formulas like Bernoulli's or the
differential equation
\begin{align}
\dv{x} f(x) = f(x)
\end{align}
/without/ the expectation that they need to mathematically evaluate any infinite
limits.
...
integrals. Ideally, American math education should try to wrestle with this in
an intelligent way.
...
\begin{align*}
\ln(x_{1}x_{2}) &= \int_{1}^{x_{1}x_{2}} \frac{1}{y}\dd{y} \\
      &= \int^{x_{1}}_{1/x_{2}} \frac{1}{y} \dd{y} \\
      &= \int_{1/x_{2}}^{1} \frac{1}{y} \dd{y} + \int_{1}^{x_{1}} \frac{1}{y} \dd{y} \\
      &= \int_{1}^{x_{2}} \frac{1}{y} \dd{y} + \int_{1}^{x_{1}} \frac{1}{y} \dd{y} \\
      &= \ln x_{2} + \ln x_{1}.
\end{align*}
...
\begin{align}
\ln(1 + x) = \int_{1}^{1+x} \frac{1}{y}\dd{y} \approx x.
\end{align}
